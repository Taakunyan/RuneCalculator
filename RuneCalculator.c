...
void consoleWriteLine(unsigned int *pair, unsigned int runeNum, struct Node *root){
    ...
    do{
        ...
        for(unsigned int i = 0; i < runeNum; i++){
            ...
            printf("\x1b[39m%2d\x1b[%dm %-20s\x1b[39m", tmp[i]->rune->level, !strcmp(tmp[i]->rune->type, "攻撃") ? 31 : !strcmp(tmp[i]->rune->type, "防御")? 32 : 39, tmp[i]->rune->name);
            tmp[i] = tmp[i]->center;
            flag = 1;
        }
        printf("\n");
...
int main(int argc, char *argv[]){
    ...
    while(1){
        ...
        if(ans)printf("%d 通り見つかりました。\n", len(ans));
        else printf("見つかりませんでした。\n");
        freeQueue(ans);
        runeNum++;
        input:
        printf("ルーンを %d 個に増やして探索しますか？(y/n) : ", runeNum);
        scanf(" %c", &moji);
        if(moji == 'n')break;
